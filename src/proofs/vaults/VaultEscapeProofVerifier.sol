...
import "./IVaultEscapeProofVerifier.sol";
import "./IVaultEscapeProofVerifier.sol";
...
contract VaultEscapeProofVerifier is IVaultEscapeProofVerifier {
    ...
    function extractLeafFromProof(uint256[] calldata escapeProof) external pure override returns (Vault memory) {
        _validateProofStructure(escapeProof);
        return _extractLeafFromProof(escapeProof);
    }
    ...
    function extractRootFromProof(uint256[] calldata escapeProof) external pure override returns (uint256) {
        _validateProofStructure(escapeProof);
        return _extractRootFromProof(escapeProof);
    }
    ...
    returns (Vault memory, uint256)
    {
        ...
        uint256 root = _extractRootFromProof(escapeProof);
        return (vault, root);
    }

    function _extractLeafFromProof(uint256[] memory proof) private pure returns (Vault memory) {
        uint256 starkKey;
        uint256 assetId;
    ...
    function _extractRootFromProof(uint256[] calldata escapeProof) private pure returns (uint256) {
