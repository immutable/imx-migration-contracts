    ...
    ReentrancyGuard
{
     ...
     * @dev Assumes that the caller is the gas refund receiver, and can receive native asset
     */
    function migrateHoldings(AssetHolding[] calldata assets)
        external
        payable
        override
        nonReentrant
    ...
    {
        require(assets.length > 0, NoAssetsProvided());

        for (uint256 i = 0; i < assets.length; i++) {
            (address asset, uint256 amount) = (assets[i].token, assets[i].amount);
            if (asset == NATIVE_ETH) {
                _depositETHToZKEVMBridge(amount, msg.value);
                emit ETHHoldingsMigration(amount, zkEVMWithdrawalProcessor, msg.sender);
            } else {
                IERC20Metadata token = IERC20Metadata(asset);
                _depositERC20ToZKEVMBridge(token, amount, msg.value);
                emit ERC20HoldingsMigration(asset, amount, zkEVMWithdrawalProcessor, msg.sender);
            }
        }
    }
    /**
